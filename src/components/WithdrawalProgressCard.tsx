...
export const WithdrawalProgressCard: React.FC = () => {
  const { user, economy, requestWithdrawal, withdrawals, setActiveTab } = useData();
          ...
          <div className="space-y-2.5">
            {withdrawals.map((wd) => {
              ...
              return (
                <div
                  key={wd.id}
                  className="p-3.5 rounded-xl bg-[#0A0A0C] border border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-white/70 shrink-0">
                      {wd.method === 'UPI' ? <QrCode className="w-4 h-4" /> : <Building2 className="w-4 h-4" />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-white font-mono">₹{wd.amount.toFixed(2)}</span>
